...
def main():
    ...
    len1a = (''.join(t) for t in itertools.product(alphabet, repeat=1))
    len1b = (''.join(t) for t in itertools.product(alphabet, repeat=1))
    len2a = (''.join(t) for t in itertools.product(alphabet, repeat=2))
    len2b = (''.join(t) for t in itertools.product(alphabet, repeat=2))

    cross1a = (''.join(t) for t in itertools.product(len1a, all_words))
    cross1b = (''.join(t) for t in itertools.product(all_words, len1b))
    cross2a = (''.join(t) for t in itertools.product(len2a, all_words))
    cross2b = (''.join(t) for t in itertools.product(all_words, len2b))

    all_candidates = itertools.chain(cross1a, cross1b, cross2a, cross2b)
        
        

    total = 2*len(all_words)*(len(alphabet)**2 + len(alphabet))
    count = 0
